...
#ifndef CEPH_HW_MESSENGER_H
#define CEPH_HW_MESSENGER_H
...
class Processor {
 ...
 public:
  Processor(HwMessenger *r, Worker *w, CephContext *c);
...
/**
 * The interface used by the HwMessenger to create hardware connections. The static 
 * method takes the type of the factory that has to be created for creating connections.
 * It is important that the type would be taken from the constructor of the Messenger, since
 * the same type of a factory may create different connections (e.g., RDMAConnFactory 
 * may create three different connections: RC-RDMA, UC-RDMA, UG-RDMA). The type is read
 * from the Ceph configuration file (type passed to messenger).
  */
class HwConnectionFactory
{
 
 public:

   HwConnectionFactory(const std::string &type);
   virtual ~HwConnectionFactory();

  /**
   * A public method that encapsulates the creation of a 
   *  hawrdware connection 
   *
   * @param c    : reference to the CephContext on which to apply method 
   * @param m    : reference to the messenger for the created connection
   * @param d    : dispatch queue used by the created connection 
   * @param w    : reference to the worker that handles the connection
   *
   * @return     : a new HwConnection
   */
 
   virtual HwConnection* create_connection(CephContext *c, HwMessenger *m, 
                                           DispatchQueue *d, Worker *w) = 0;


   /**
    * Methods returns the type of this factory.
    * The return string encodes info about the underlying
    * hardware of the connections created by this factory
    *
    * @return : string that describes the hardware (e.g., "rdma")
    */
   const std::string& getFactoryType() const  { return hw_type; }


   /**
    * A Factory method for creating hardware connection
    *  factories.
    *
    * @param type : string from the consturctor of HwMessenger
    */
   static  HwConnectionFacotry* create_factory(const std::string &type);  
...
/*
 * HwMessenger is a more concrete interface for hardware connections 
 * than AsyncMessenger. The basic data flow and architecture of the 
 * messenger is the same AsyncMessenger's. However, HwMessenger adds
 * some abstracton to its managed connections so that lower-level
 * code could take advantage of the hardware more effieciently. 
 */

class HwMessenger : public SimplePolicyMessenger {
  // First we have the public Messenger interface implementation...
public:
  /**
   * Initialize the HwMessenger!
   *
   * @param cct The CephContext to use
   * @param name The name to assign ourselves
   * _nonce A unique ID to use for this HwMessenger. It should not
   * be a value that will be repeated if the daemon restarts.
   */
  HwMessenger(CephContext *cct, entity_name_t name, const std::string &type,
                 string mname, uint64_t _nonce);

  /**
   * Destroy the HwMessenger. Pretty simple since all the work is done
   * elsewhere.
   */
  ~HwMessenger() override;
...
private:
   ...
   */
  HwConnectionRef create_connect(const entity_addr_t& addr, int type);

  /**
   * Queue up a Message for delivery to the entity specified
   * by addr and dest_type.
   * submit_message() is responsible for creating
   * new HwConnection (and closing old ones) as necessary.
   *
   ...
   */
  void submit_message(Message *m, HwConnectionRef con,
 ...
 private:
  ...
  /// overall lock used for HwMessenger data structures
  Mutex lock;
  // HwMessenger stuff
  ...
  /**
   *  false; set to true if the HwMessenger bound to a specific address;
   ...
   */
  ceph::unordered_map<entity_addr_t, HwConnectionRef> conns;
   ...
   */
  set<HwConnectionRef> accepting_conns;

  /**
   * list of connection are closed which need to be clean up
   *
   * Because HwMessenger and HwConnection follow a lock rule that
   * we can lock AsyncMesenger::lock firstly then lock HwConnection::lock
   * but can't reversed. This rule is aimed to avoid dead lock.
   * So if HwConnection want to unregister itself from HwMessenger,
   * we pick up this idea that just queue itself to this set and do lazy
   * deleted for HwConnection. "_lookup_conn" must ensure not return a
   * HwConnection in this set.
   */
  Mutex deleted_lock;
  set<HwConnectionRef> deleted_conns;
  ...
  HwConnectionRef _lookup_conn(const entity_addr_t& k) {
    assert(lock.is_locked());
    ceph::unordered_map<entity_addr_t, HwConnectionRef>::iterator p = conns.find(k);
    if (p == conns.end())
      return NULL;
...
public:
  ...
  /**
   * @defgroup HwMessenger internals
   * @{
   */
  /**
   * This wraps _lookup_conn.
   */
  HwConnectionRef lookup_conn(const entity_addr_t& k) {
    Mutex::Locker l(lock);
    return _lookup_conn(k);
  }

  int accept_conn(HwConnectionRef conn) {
    Mutex::Locker l(lock);
    auto it = conns.find(conn->peer_addr);
    if (it != conns.end()) {
      HwConnectionRef existing = it->second;
  ...
  /**
   * This wraps ms_deliver_get_authorizer. We use it for HwConnection.
   */
  ...
  /**
   * This wraps ms_deliver_verify_authorizer; we use it for HwConnection.
   */
  ...
  /**
   * Increment the global sequence for this HwMessenger and return it.
   ...
   * See "deleted_conns"
   */
  void unregister_conn(HwConnectionRef conn) {
    Mutex::Locker l(deleted_lock);
