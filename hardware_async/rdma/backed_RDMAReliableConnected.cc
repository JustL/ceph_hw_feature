...
const int RDMAReliableConnected::TCP_PREFETCH_MIN_SIZE = 512;
...
RDMAReliableConnected::RDMAReliableConnected(CephContext *cct, HwMessenger *m, DispatchQueue *q,
                                 Worker *w)
  : Connection(cct, m), delay_state(NULL), hw_msgr(m), conn_id(q->get_id()),
...
ssize_t RDMAReliableConnected::read_bulk(char *buf, unsigned len)
{
  ssize_t nread;
 again:
  ...
  if (nread < 0) {
    if (nread == -EAGAIN) {
      ...
      ldout(hw_msgr->cct, 1) << __func__ << " reading from fd=" << cs.fd()
                          << " : "<< strerror(nread) << dendl;
      return -1;
    }
  } else if (nread == 0) {
    ldout(hw_msgr->cct, 1) << __func__ << " peer close file descriptor "
                              << cs.fd() << dendl;
    return -1;
  }
  return nread;
...
ssize_t RDMAReliableConnected::_try_send(bool more)
{
  if (hw_msgr->cct->_conf->ms_inject_socket_failures && cs) {
    if (rand() % hw_msgr->cct->_conf->ms_inject_socket_failures == 0) {
      ldout(hw_msgr->cct, 0) << __func__ << " injecting socket failure" << dendl;
      cs.shutdown();
    }
  }
  ...
  if (r < 0) {
    ldout(hw_msgr->cct, 1) << __func__ << " send error: " << cpp_strerror(r) << dendl;
    return r;
  }

  ldout(hw_msgr->cct, 10) << __func__ << " sent bytes " << r
...
// else return < 0 means error
ssize_t RDMAReliableConnected::read_until(unsigned len, char *p)
{
  ldout(hw_msgr->cct, 25) << __func__ << " len is " << len << " state_offset is "
                             << state_offset << dendl;
  ...
  if (recv_end > recv_start) {
    ...
    recv_start += to_read;
    left -= to_read;
    ldout(hw_msgr->cct, 25) << __func__ << " got " << to_read << " in buffer "
  ...
  if (len > recv_max_prefetch) {
    ...
    do {
      ...
      ldout(hw_msgr->cct, 25) << __func__ << " read_bulk left is " << left << " got " << r << dendl;
      if (r < 0) {
        ldout(hw_msgr->cct, 1) << __func__ << " read failed" << dendl;
        return -1;
    ...
    do {
      ...
      ldout(hw_msgr->cct, 25) << __func__ << " read_bulk recv_end is " << recv_end
  ...
  ldout(hw_msgr->cct, 25) << __func__ << " need len " << len << " remaining "
                             << len - state_offset << " bytes" << dendl;
  return len - state_offset;
}

void RDMAReliableConnected::inject_delay() {
  if (hw_msgr->cct->_conf->ms_inject_internal_delays) {
    ldout(hw_msgr->cct, 10) << __func__ << " sleep for " << 
      hw_msgr->cct->_conf->ms_inject_internal_delays << dendl;
    utime_t t;
    t.set_from_double(hw_msgr->cct->_conf->ms_inject_internal_delays);
    t.sleep();
  }
}
...
void RDMAReliableConnected::process()
{
  ...
  do {
    ldout(hw_msgr->cct, 20) << __func__ << " prev state is " << get_state_name(prev_state) << dendl;
    prev_state = state;
    switch (state) {
      case STATE_OPEN:
        {
          ...
          if (r < 0) {
            ldout(hw_msgr->cct, 1) << __func__ << " read tag failed" << dendl;
            goto fail;
          } else if (r > 0) {
            break;
          }
          ...
          if (tag == CEPH_MSGR_TAG_KEEPALIVE) {
            ldout(hw_msgr->cct, 20) << __func__ << " got KEEPALIVE" << dendl;
	    set_last_keepalive(ceph_clock_now());
            ...
            ldout(hw_msgr->cct, 0) << __func__ << " bad tag " << (int)tag << dendl;
            goto fail;
          }
      ...
      case STATE_OPEN_KEEPALIVE2:
        {
          ...
          if (r < 0) {
            ldout(hw_msgr->cct, 1) << __func__ << " read keeplive timespec failed" << dendl;
            goto fail;
          } else if (r > 0) {
            break;
          }

          ldout(hw_msgr->cct, 30) << __func__ << " got KEEPALIVE2 tag ..." << dendl;
          t = (ceph_timespec*)state_buffer;
          utime_t kp_t = utime_t(*t);
          write_lock.lock();
          _append_keepalive_or_ack(true, &kp_t);
	  write_lock.unlock();
          ldout(hw_msgr->cct, 20) << __func__ << " got KEEPALIVE2 " << kp_t << dendl;
	  set_last_keepalive(ceph_clock_now());
      ...
      case STATE_OPEN_KEEPALIVE2_ACK:
        {
          ...
          ldout(hw_msgr->cct, 20) << __func__ << " got KEEPALIVE_ACK" << dendl;
          state = STATE_OPEN;
          break;
        }
      ...
      case STATE_OPEN_TAG_ACK:
        {
          ...
          if (r < 0) {
            ldout(hw_msgr->cct, 1) << __func__ << " read ack seq failed" << dendl;
            goto fail;
          } else if (r > 0) {
            break;
          }
          ...
          ldout(hw_msgr->cct, 20) << __func__ << " got ACK" << dendl;
          handle_ack(*seq);
          state = STATE_OPEN;
          break;
        }
      ...
      case STATE_OPEN_MESSAGE_HEADER:
        {
          ...
          ldout(hw_msgr->cct, 20) << __func__ << " begin MSG" << dendl;
          ...
          if (r < 0) {
            ldout(hw_msgr->cct, 1) << __func__ << " read message header failed" << dendl;
            goto fail;
          } else if (r > 0) {
            break;
          }

          ldout(hw_msgr->cct, 20) << __func__ << " got MSG header" << dendl;
          ...
          ldout(hw_msgr->cct, 20) << __func__ << " got envelope type=" << header.type
          ...
          if (msgr->crcflags & MSG_CRC_HEADER && header_crc != header.crc) {
            ldout(hw_msgr->cct,0) << __func__ << " got bad header crc "
      ...
      case STATE_OPEN_MESSAGE_THROTTLE_MESSAGE:
        {
          if (policy.throttler_messages) {
            ldout(hw_msgr->cct, 10) << __func__ << " wants " << 1 << " message from policy throttler "
                                       << policy.throttler_messages->get_current() << "/"
                                       << policy.throttler_messages->get_max() << dendl;
            if (!policy.throttler_messages->get_or_fail()) {
              ldout(hw_msgr->cct, 10) << __func__ << " wants 1 message from policy throttle "
					 << policy.throttler_messages->get_current() << "/"
      ...
      case STATE_OPEN_MESSAGE_THROTTLE_BYTES:
        {
          ...
          if (cur_msg_size) {
            if (policy.throttler_bytes) {
              ldout(hw_msgr->cct, 10) << __func__ << " wants " << cur_msg_size << " bytes from policy throttler "
      ...
      case STATE_OPEN_MESSAGE_THROTTLE_DISPATCH_QUEUE:
        {
          if (cur_msg_size) {
            if (!dispatch_queue->dispatch_throttler.get_or_fail(cur_msg_size)) {
              ldout(hw_msgr->cct, 10) << __func__ << " wants " << cur_msg_size << " bytes from dispatch throttle "
      ...
      case STATE_OPEN_MESSAGE_READ_FRONT:
        {
          ...
          if (front_len) {
            ...
            if (r < 0) {
              ldout(hw_msgr->cct, 1) << __func__ << " read message front failed" << dendl;
              goto fail;
            } else if (r > 0) {
              break;
            }

            ldout(hw_msgr->cct, 20) << __func__ << " got front " << front.length() << dendl;
          }
      ...
      case STATE_OPEN_MESSAGE_READ_MIDDLE:
        {
          ...
          if (middle_len) {
            ...
            if (r < 0) {
              ldout(hw_msgr->cct, 1) << __func__ << " read message middle failed" << dendl;
              goto fail;
            } else if (r > 0) {
              break;
            }
            ldout(hw_msgr->cct, 20) << __func__ << " got middle " << middle.length() << dendl;
          }
      ...
      case STATE_OPEN_MESSAGE_READ_DATA_PREPARE:
        {
          ...
          if (data_len) {
            ...
            if (p != rx_buffers.end()) {
              ldout(hw_msgr->cct,10) << __func__ << " seleting rx buffer v " << p->second.second
                                  << " at offset " << data_off
              ...
              ldout(hw_msgr->cct,20) << __func__ << " allocating new rx buffer at offset " << data_off << dendl;
      ...
      case STATE_OPEN_MESSAGE_READ_DATA:
        {
          while (msg_left > 0) {
            ...
            if (r < 0) {
              ldout(hw_msgr->cct, 1) << __func__ << " read data error " << dendl;
              goto fail;
            } else if (r > 0) {
              break;
            }
      ...
      case STATE_OPEN_MESSAGE_READ_FOOTER_AND_DISPATCH:
        {
          ...
          r = read_until(len, state_buffer);
          if (r < 0) {
            ldout(hw_msgr->cct, 1) << __func__ << " read footer data error " << dendl;
            goto fail;
          } else if (r > 0) {
            break;
          }
          ...
          ldout(hw_msgr->cct, 10) << __func__ << " aborted = " << aborted << dendl;
          if (aborted) {
            ldout(hw_msgr->cct, 0) << __func__ << " got " << front.length() << " + " << middle.length() << " + " << data.length()
                                << " byte message.. ABORTED" << dendl;
            goto fail;
          }

          ldout(hw_msgr->cct, 20) << __func__ << " got " << front.length() << " + " << middle.length()
                              << " + " << data.length() << " byte message" << dendl;
          Message *message = decode_message(hw_msgr->cct, async_msgr->crcflags, current_header, footer,
                                            front, middle, data, this);
          if (!message) {
            ldout(hw_msgr->cct, 1) << __func__ << " decode message failed " << dendl;
            goto fail;
          }
          ...
          if (session_security.get() == NULL) {
            ldout(hw_msgr->cct, 10) << __func__ << " no session security set" << dendl;
          } else {
            if (session_security->check_message_signature(message)) {
              ldout(hw_msgr->cct, 0) << __func__ << " Signature check failed" << dendl;
              message->put();
              goto fail;
            }
          }
          ...
          if (message->get_seq() <= cur_seq) {
            ldout(hw_msgr->cct,0) << __func__ << " got old message "
                    << message->get_seq() << " <= " << cur_seq << " " << message << " " << *message
                    << ", discarding" << dendl;
            message->put();
            if (has_feature(CEPH_FEATURE_RECONNECT_SEQ) && hw_msgr->cct->_conf->ms_die_on_old_message)
              assert(0 == "old msgs despite reconnect_seq feature");
            break;
          }
          if (message->get_seq() > cur_seq + 1) {
            ldout(hw_msgr->cct, 0) << __func__ << " missed message?  skipped from seq "
                                      << cur_seq << " to " << message->get_seq() << dendl;
            if (hw_msgr->cct->_conf->ms_die_on_skipped_message)
	  ...
	  ldout(hw_msgr->cct, 5) << " rx " << message->get_source() << " seq "
          ...
          hw_msgr->ms_fast_preprocess(message);
          ...
          if (delay_state) {
            ...
            if (rand() % 10000 < hw_msgr->cct->_conf->ms_inject_delay_probability * 10000.0) {
              delay_period = hw_msgr->cct->_conf->ms_inject_delay_max * (double)(rand() % 10000) / 10000.0;
              release += delay_period;
              ldout(hw_msgr->cct, 1) << "queue_received will delay until " << release << " on "
                                        << message << " " << *message << dendl;
            }
            delay_state->queue(delay_period, release, message);
          } else if (hw_msgr->ms_can_fast_dispatch(message)) {
            lock.unlock();
      ...
      case STATE_OPEN_TAG_CLOSE:
        {
          ldout(hw_msgr->cct, 20) << __func__ << " got CLOSE" << dendl;
          _stop();
          return ;
        }
      ...
      case STATE_STANDBY:
        {
          ldout(hw_msgr->cct, 20) << __func__ << " enter STANDY" << dendl;
      ...
      case STATE_NONE:
        {
          ldout(hw_msgr->cct, 20) << __func__ << " enter none state" << dendl;
          break;
        }

      case STATE_CLOSED:
        {
          ldout(hw_msgr->cct, 20) << __func__ << " socket closed" << dendl;
          break;
        }

      case STATE_WAIT:
        {
          ldout(hw_msgr->cct, 1) << __func__ << " enter wait state, failing" << dendl;
          goto fail;
        }
...
ssize_t RDMAReliableConnected::_process_connection()
{
  ssize_t r = 0;
  ...
  switch(state) {
    ...
    case STATE_CONNECTING:
      {
        ...
        SocketOptions opts;
        opts.priority = hw_msgr->get_socket_priority();
    ...
    case STATE_CONNECTING_RE:
      {
        ...
        if (r < 0) {
          ...
          ldout(hw_msgr->cct, 10) << __func__ << " nonblock connect inprogress" << dendl;
        ...
        center->delete_file_event(cs.fd(), EVENT_WRITABLE);
        ldout(hw_msgr->cct, 10) << __func__ << " connect successfully, ready to send banner" << dendl;
        ...
        if (r == 0) {
          ...
          ldout(hw_msgr->cct, 10) << __func__ << " connect write banner done: "
                                     << get_peer_addr() << dendl;
        } else if (r > 0) {
          state = STATE_WAIT_SEND;
          state_after_send = STATE_CONNECTING_WAIT_BANNER_AND_IDENTIFY;
          ldout(hw_msgr->cct, 10) << __func__ << " connect wait for write banner: "
                               << get_peer_addr() << dendl;
        } else {
          goto fail;
        }
    ...
    case STATE_CONNECTING_WAIT_BANNER_AND_IDENTIFY:
      {
        ...
        if (r < 0) {
          ldout(hw_msgr->cct, 1) << __func__ << " read banner and identify addresses failed" << dendl;
          goto fail;
        } else if (r > 0) {
          break;
        }
        ...
        if (memcmp(state_buffer, CEPH_BANNER, banner_len)) {
          ldout(hw_msgr->cct, 0) << __func__ << " connect protocol error (bad banner) on peer "
                                    << get_peer_addr() << dendl;
          goto fail;
        }
        ...
        ldout(hw_msgr->cct, 20) << __func__ <<  " connect read peer addr "
                             << paddr << " on socket " << cs.fd() << dendl;
        if (peer_addr != paddr) {
          if (paddr.is_blank_ip() && peer_addr.get_port() == paddr.get_port() &&
              peer_addr.get_nonce() == paddr.get_nonce()) {
            ldout(hw_msgr->cct, 0) << __func__ <<  " connect claims to be " << paddr
                                << " not " << peer_addr
                                << " - presumably this is the same node!" << dendl;
          } else {
            ldout(hw_msgr->cct, 10) << __func__ << " connect claims to be "
				       << paddr << " not " << peer_addr << dendl;
	    goto fail;
          }
        }

        ldout(hw_msgr->cct, 20) << __func__ << " connect peer addr for me is " << peer_addr_for_me << dendl;
        lock.unlock();
        ...
        if (state != STATE_CONNECTING_WAIT_BANNER_AND_IDENTIFY) {
          ldout(hw_msgr->cct, 1) << __func__ << " state changed while learned_addr, mark_down or "
                                    << " replacing must be happened just now" << dendl;
          return 0;
        }

        ::encode(hw_msgr->get_myaddr(), myaddrbl, 0); // legacy
        r = try_send(myaddrbl);
        if (r == 0) {
          state = STATE_CONNECTING_SEND_CONNECT_MSG;
          ldout(hw_msgr->cct, 10) << __func__ << " connect sent my addr "
              << hw_msgr->get_myaddr() << dendl;
        } else if (r > 0) {
          state = STATE_WAIT_SEND;
          state_after_send = STATE_CONNECTING_SEND_CONNECT_MSG;
          ldout(hw_msgr->cct, 10) << __func__ << " connect send my addr done: "
              << hw_msgr->get_myaddr() << dendl;
        } else {
          ldout(hw_msgr->cct, 2) << __func__ << " connect couldn't write my addr, "
    ...
    case STATE_CONNECTING_SEND_CONNECT_MSG:
      {
        if (!got_bad_auth) {
          delete authorizer;
          authorizer = hw_msgr->get_authorizer(peer_type, false);
        }
        bufferlist bl;

        connect_msg.features = policy.features_supported;
        connect_msg.host_type = hw_msgr->get_myinst().name.type();
        connect_msg.global_seq = global_seq;
        connect_msg.connect_seq = connect_seq;
        connect_msg.protocol_version = hw_msgr->get_proto_version(peer_type, true);
        ...
        if (authorizer)
          ldout(hw_msgr->cct, 10) << __func__ <<  " connect_msg.authorizer_len="
        ...
        ldout(hw_msgr->cct, 10) << __func__ << " connect sending gseq=" << global_seq << " cseq="
            << connect_seq << " proto=" << connect_msg.protocol_version << dendl;

        r = try_send(bl);
        if (r == 0) {
          state = STATE_CONNECTING_WAIT_CONNECT_REPLY;
          ldout(hw_msgr->cct,20) << __func__ << " connect wrote (self +) cseq, waiting for reply" << dendl;
        } else if (r > 0) {
          state = STATE_WAIT_SEND;
          state_after_send = STATE_CONNECTING_WAIT_CONNECT_REPLY;
          ldout(hw_msgr->cct, 10) << __func__ << " continue send reply " << dendl;
        } else {
          ldout(hw_msgr->cct, 2) << __func__ << " connect couldn't send reply "
              << cpp_strerror(r) << dendl;
          goto fail;
        }
    ...
    case STATE_CONNECTING_WAIT_CONNECT_REPLY:
      {
        ...
        if (r < 0) {
          ldout(hw_msgr->cct, 1) << __func__ << " read connect reply failed" << dendl;
          goto fail;
        } else if (r > 0) {
          break;
        }
        ...
        ldout(hw_msgr->cct, 20) << __func__ << " connect got reply tag " << (int)connect_reply.tag
    ...
    case STATE_CONNECTING_WAIT_CONNECT_REPLY_AUTH:
      {
        ...
        if (connect_reply.authorizer_len) {
          ldout(hw_msgr->cct, 10) << __func__ << " reply.authorizer_len=" << connect_reply.authorizer_len << dendl;
          assert(connect_reply.authorizer_len < 4096);
          r = read_until(connect_reply.authorizer_len, state_buffer);
          if (r < 0) {
            ldout(hw_msgr->cct, 1) << __func__ << " read connect reply authorizer failed" << dendl;
            goto fail;
          } else if (r > 0) {
            break;
          }
          ...
          if (authorizer && !authorizer->verify_reply(iter)) {
            ldout(hw_msgr->cct, 0) << __func__ << " failed verifying authorize reply" << dendl;
            goto fail;
          }
        }
    ...
    case STATE_CONNECTING_WAIT_ACK_SEQ:
      {
        ...
        if (r < 0) {
          ldout(hw_msgr->cct, 1) << __func__ << " read connect ack seq failed" << dendl;
          goto fail;
        } else if (r > 0) {
          break;
        }
        ...
        ldout(hw_msgr->cct, 2) << __func__ << " got newly_acked_seq " << newly_acked_seq
                            << " vs out_seq " << out_seq << dendl;
        discard_requeued_up_to(newly_acked_seq);
        //while (newly_acked_seq > out_seq.read()) {
        //  Message *m = _get_next_outgoing(NULL);
        //  assert(m);
        //  ldout(hw_msgr->cct, 2) << __func__ << " discarding previously sent " << m->get_seq()
        //                      << " " << *m << dendl;
        ...
        if (r == 0) {
          ...
          ldout(hw_msgr->cct, 10) << __func__ << " send in_seq done " << dendl;
        } else if (r > 0) {
          state_after_send = STATE_CONNECTING_READY;
          state = STATE_WAIT_SEND;
          ldout(hw_msgr->cct, 10) << __func__ << " continue send in_seq " << dendl;
        } else {
          goto fail;
        }
        break;
    ...
    case STATE_CONNECTING_READY:
      {
        ...
        ldout(hw_msgr->cct, 10) << __func__ << " connect success " << connect_seq
        ...
        if (authorizer != NULL) {
          session_security.reset(
              get_auth_session_handler(hw_msgr->cct,
                                       authorizer->protocol,
                                       authorizer->session_key,
                                       get_features()));
        } else {
          // We have no authorizer, so we shouldn't be applying security to messages in this RDMAReliableConnected.  PLR
          session_security.reset();
        }
        ...
        hw_msgr->ms_deliver_handle_fast_connect(this);
    ...
    case STATE_ACCEPTING:
      {
        ...
        ::encode(hw_msgr->get_myaddr(), bl, 0); // legacy
        port = hw_msgr->get_myaddr().get_port();
        ::encode(socket_addr, bl, 0); // legacy
        ldout(hw_msgr->cct, 1) << __func__ << " sd=" << cs.fd() << " " << socket_addr << dendl;

        r = try_send(bl);
        if (r == 0) {
          state = STATE_ACCEPTING_WAIT_BANNER_ADDR;
          ldout(hw_msgr->cct, 10) << __func__ << " write banner and addr done: "
            << get_peer_addr() << dendl;
        } else if (r > 0) {
          state = STATE_WAIT_SEND;
          state_after_send = STATE_ACCEPTING_WAIT_BANNER_ADDR;
          ldout(hw_msgr->cct, 10) << __func__ << " wait for write banner and addr: "
                              << get_peer_addr() << dendl;
        } else {
          goto fail;
        }
    ...
    case STATE_ACCEPTING_WAIT_BANNER_ADDR:
      {
        ...
        if (r < 0) {
          ldout(hw_msgr->cct, 1) << __func__ << " read peer banner and addr failed" << dendl;
          goto fail;
        } else if (r > 0) {
          break;
        }
        ...
        if (memcmp(state_buffer, CEPH_BANNER, strlen(CEPH_BANNER))) {
          ldout(hw_msgr->cct, 1) << __func__ << " accept peer sent bad banner '" << state_buffer
	  ...
	  lderr(hw_msgr->cct) << __func__ <<  " decode peer_addr failed " << dendl;
          goto fail;
	}

        ldout(hw_msgr->cct, 10) << __func__ << " accept peer addr is " << peer_addr << dendl;
        if (peer_addr.is_blank_ip()) {
          // peer apparently doesn't know what ip they have; figure it out for them.
          int port = peer_addr.get_port();
          peer_addr.u = socket_addr.u;
          peer_addr.set_port(port);
          ldout(hw_msgr->cct, 0) << __func__ << " accept peer addr is really " << peer_addr
    ...
    case STATE_ACCEPTING_WAIT_CONNECT_MSG:
      {
        ...
        if (r < 0) {
          ldout(hw_msgr->cct, 1) << __func__ << " read connect msg failed" << dendl;
          goto fail;
        } else if (r > 0) {
          break;
        }
    ...
    case STATE_ACCEPTING_WAIT_CONNECT_MSG_AUTH:
      {
        ...
        if (connect_msg.authorizer_len) {
          ...
          if (r < 0) {
            ldout(hw_msgr->cct, 1) << __func__ << " read connect authorizer failed" << dendl;
            goto fail;
          } else if (r > 0) {
            break;
          }
        ...
        ldout(hw_msgr->cct, 20) << __func__ << " accept got peer connect_seq "
                             << connect_msg.connect_seq << " global_seq "
                             << connect_msg.global_seq << dendl;
        set_peer_type(connect_msg.host_type);
        policy = hw_msgr->get_policy(connect_msg.host_type);
        ldout(hw_msgr->cct, 10) << __func__ << " accept of host_type " << connect_msg.host_type
    ...
    case STATE_ACCEPTING_WAIT_SEQ:
      {
        ...
        newly_acked_seq = *((uint64_t*)state_buffer);
        ldout(hw_msgr->cct, 2) << __func__ << " accept get newly_acked_seq " << newly_acked_seq << dendl;
        discard_requeued_up_to(newly_acked_seq);
    ...
    case STATE_ACCEPTING_READY:
      {
        ldout(hw_msgr->cct, 20) << __func__ << " accept done" << dendl;
        state = STATE_OPEN;
        memset(&connect_msg, 0, sizeof(connect_msg));
    ...
    default:
      {
        lderr(hw_msgr->cct) << __func__ << " bad state: " << state << dendl;
        ceph_abort();
      }
  }
...
fail_registered:
  ldout(hw_msgr->cct, 10) << "accept fault after register" << dendl;
  inject_delay();
...
int RDMAReliableConnected::handle_connect_reply(ceph_msg_connect &connect, ceph_msg_connect_reply &reply)
{
  uint64_t feat_missing;
  if (reply.tag == CEPH_MSGR_TAG_FEATURES) {
    ldout(hw_msgr->cct, 0) << __func__ << " connect protocol feature mismatch, my "
  ...
  if (reply.tag == CEPH_MSGR_TAG_BADPROTOVER) {
    ldout(hw_msgr->cct, 0) << __func__ << " connect protocol version mismatch, my "
  ...
  if (reply.tag == CEPH_MSGR_TAG_BADAUTHORIZER) {
    ldout(hw_msgr->cct,0) << __func__ << " connect got BADAUTHORIZER" << dendl;
    if (got_bad_auth)
      goto fail;
    got_bad_auth = true;
    delete authorizer;
    authorizer = hw_msgr->get_authorizer(peer_type, true);  // try harder
    state = STATE_CONNECTING_SEND_CONNECT_MSG;
  }
  if (reply.tag == CEPH_MSGR_TAG_RESETSESSION) {
    ldout(hw_msgr->cct, 0) << __func__ << " connect got RESETSESSION" << dendl;
    was_session_reset();
  ...
  if (reply.tag == CEPH_MSGR_TAG_RETRY_GLOBAL) {
    global_seq = hw_msgr->get_global_seq(reply.global_seq);
    ldout(hw_msgr->cct, 5) << __func__ << " connect got RETRY_GLOBAL "
  ...
  if (reply.tag == CEPH_MSGR_TAG_RETRY_SESSION) {
    ...
    ldout(hw_msgr->cct, 5) << __func__ << " connect got RETRY_SESSION "
                              << connect_seq << " -> "
  ...
  if (reply.tag == CEPH_MSGR_TAG_WAIT) {
    ldout(hw_msgr->cct, 1) << __func__ << " connect got WAIT (connection race)" << dendl;
    state = STATE_WAIT;
  }

  feat_missing = policy.features_required & ~(uint64_t)connect_reply.features;
  if (feat_missing) {
    ldout(hw_msgr->cct, 1) << __func__ << " missing required features " << std::hex
                              << feat_missing << std::dec << dendl;
    goto fail;
  }

  if (reply.tag == CEPH_MSGR_TAG_SEQ) {
    ldout(hw_msgr->cct, 10) << __func__ << " got CEPH_MSGR_TAG_SEQ, reading acked_seq and writing in_seq" << dendl;
    state = STATE_CONNECTING_WAIT_ACK_SEQ;
  }
  if (reply.tag == CEPH_MSGR_TAG_READY) {
    ldout(hw_msgr->cct, 10) << __func__ << " got CEPH_MSGR_TAG_READY " << dendl;
    state = STATE_CONNECTING_READY;
  }
...
ssize_t RDMAReliableConnected::handle_connect_msg(ceph_msg_connect &connect, bufferlist &authorizer_bl,
                                            bufferlist &authorizer_reply)
{
  ssize_t r = 0;
  ...
  reply.protocol_version = hw_msgr->get_proto_version(peer_type, false);

  // mismatch?
  ldout(hw_msgr->cct, 10) << __func__ << " accept my proto " << reply.protocol_version
  ...
  if (connect.authorizer_protocol == CEPH_AUTH_CEPHX) {
    if (peer_type == CEPH_ENTITY_TYPE_OSD ||
      ...
      if (hw_msgr->cct->_conf->cephx_require_signatures ||
          hw_msgr->cct->_conf->cephx_cluster_require_signatures) {
        ldout(hw_msgr->cct, 10) << __func__ << " using cephx, requiring MSG_AUTH feature bit for cluster" << dendl;
        policy.features_required |= CEPH_FEATURE_MSG_AUTH;
      }
    } else {
      if (hw_msgr->cct->_conf->cephx_require_signatures ||
          hw_msgr->cct->_conf->cephx_service_require_signatures) {
        ldout(hw_msgr->cct, 10) << __func__ << " using cephx, requiring MSG_AUTH feature bit for service" << dendl;
        policy.features_required |= CEPH_FEATURE_MSG_AUTH;
      }
    }
  }
  ...
  if (feat_missing) {
    ldout(hw_msgr->cct, 1) << __func__ << " peer missing required features "
  ...
  if (!hw_msgr->verify_authorizer(this, peer_type, connect.authorizer_protocol, authorizer_bl,
                               authorizer_reply, authorizer_valid, session_key) || !authorizer_valid) {
    lock.lock();
    ldout(hw_msgr->cct,0) << __func__ << ": got bad authorizer" << dendl;
    session_security.reset();
    return _reply_accept(CEPH_MSGR_TAG_BADAUTHORIZER, connect, reply, authorizer_reply);
  }

  // We've verified the authorizer for this RDMAReliableConnected, so set up the session security structure.  PLR
  ldout(hw_msgr->cct, 10) << __func__ << " accept setting up session_security." << dendl;

  // existing RDMA relibeld connected?
  RDMAReliableConnectedRef existing = dynamic_cast<RDMAReliableConectedRef>(hw_msgr->lookup_conn(peer_addr));

  inject_delay();

  lock.lock();
  if (state != STATE_ACCEPTING_WAIT_CONNECT_MSG_AUTH) {
    ldout(hw_msgr->cct, 1) << __func__ << " state changed while accept, it must be mark_down" << dendl;
  ...
  if (existing) 
  {
    ...
    // connection's lock
    existing->lock.lock();  // skip lockdep check (we are locking a second RDMAReliableConnected here)

    if (existing->state == STATE_CLOSED) {
      ldout(hw_msgr->cct, 1) << __func__ << " existing already closed." << dendl;
    ...
    if (existing->replacing) {
      ldout(hw_msgr->cct, 1) << __func__ << " existing racing replace happened while replacing."
    ...
    if (connect.global_seq < existing->peer_global_seq) {
      ldout(hw_msgr->cct, 10) << __func__ << " accept existing " << existing
    ...
    if (existing->policy.lossy) {
      ldout(hw_msgr->cct, 0) << __func__ << " accept replacing existing (lossy) channel (new one lossy="
                          << policy.lossy << ")" << dendl;
      existing->was_session_reset();
      goto replace;
    }

    ldout(hw_msgr->cct, 0) << __func__ << " accept connect_seq " << connect.connect_seq
    ...
    if (connect.connect_seq == 0 && existing->connect_seq > 0) {
      ldout(hw_msgr->cct,0) << __func__ << " accept peer reset, then tried to connect to us, replacing" << dendl;
    ...
    if (connect.connect_seq < existing->connect_seq) {
      ...
      ldout(hw_msgr->cct, 10) << __func__ << " accept existing " << existing << ".cseq "
    ...
    if (connect.connect_seq == existing->connect_seq) {
      ...
      if (existing->state == STATE_OPEN ||
        ...
        ldout(hw_msgr->cct, 10) << __func__ << " accept connection race, existing " << existing
      ...
      // connection race?
      if (peer_addr < hw_msgr->get_myaddr() || existing->policy.server) {
        // incoming wins
        ldout(hw_msgr->cct, 10) << __func__ << " accept connection race, existing " << existing
        ...
        ldout(hw_msgr->cct,10) << __func__ << " accept connection race, existing "
                            << existing << ".cseq " << existing->connect_seq
                            << " == " << connect.connect_seq << ", sending WAIT" << dendl;
        assert(peer_addr > hw_msgr->get_myaddr());
        existing->lock.unlock();
    ...
    if (policy.resetcheck &&   // RESETSESSION only used by servers; peers do not reset each other
      ...
      ldout(hw_msgr->cct, 0) << __func__ << " accept we reset (peer sent cseq "
    ...
    // reconnect
    ldout(hw_msgr->cct, 10) << __func__ << " accept peer sent cseq " << connect.connect_seq
    ...
    // new session
    ldout(hw_msgr->cct, 10) << __func__ << " accept new session" << dendl;
    existing = NULL;
    goto open;
  }
  ceph_abort();
 ...
 replace:
  ldout(hw_msgr->cct, 10) << __func__ << " accept replacing " << existing << dendl;

  inject_delay();
  if (existing->policy.lossy) {
    // disconnect from the Connection
    ldout(hw_msgr->cct, 1) << __func__ << " replacing on lossy channel, failing existing" << dendl;
    existing->_stop();
    ...
    ldout(hw_msgr->cct, 1) << __func__ << " stop myself to swap existing" << dendl;
 ...
 open:
  ...
  ldout(hw_msgr->cct, 10) << __func__ << " accept success, connect_seq = "
  ...
  // send READY reply
  reply.features = policy.features_supported;
  reply.global_seq = hw_msgr->get_global_seq();
  ...
  ldout(hw_msgr->cct, 10) << __func__ << " accept features " << get_features() << dendl;

  session_security.reset(
      get_auth_session_handler(hw_msgr->cct, connect.authorizer_protocol,
  ...
  replacing = false;
  if (r < 0) {
    ldout(hw_msgr->cct, 1) << __func__ << " existing race replacing process for addr=" << peer_addr
                              << " just fail later one(this)" << dendl;
    goto fail_registered;
  }
  if (state != STATE_ACCEPTING_WAIT_CONNECT_MSG_AUTH) {
    ldout(hw_msgr->cct, 1) << __func__ << " state changed while accept_conn, it must be mark_down" << dendl;
    assert(state == STATE_CLOSED);
    goto fail_registered;
  }
  ...
  hw_msgr->ms_deliver_handle_fast_accept(this);
  once_ready = true;

  if (r == 0) {
    state = next_state;
    ldout(hw_msgr->cct, 2) << __func__ << " accept write reply msg done" << dendl;
  } else {
    state = STATE_WAIT_SEND;
 ...
 fail_registered:
  ldout(hw_msgr->cct, 10) << __func__ << " accept fault after register" << dendl;
  inject_delay();

 fail:
  ldout(hw_msgr->cct, 10) << __func__ << " failed to accept." << dendl;
  return -1;
}
...
{
  ldout(hw_msgr->cct, 10) << __func__ << " sd=" << socket.fd() << dendl;
  assert(socket.fd() >= 0);

  std::lock_guard<std::mutex> l(lock);
  // check if the type of the socket is RDMASocket; if not, close the socket
  
  // ################# Take a look at stack.h ##############
 // need to change the ConnectedSocket so that I could
 // use an RDMA socket
...
int RDMAReliableConnected::send_message(Message *m)
{
  FUNCTRACE();
  lgeneric_subdout(hw_msgr->cct, ms,
		   1) << "-- " << hw_msgr->get_myaddr() << " --> "
		      << get_peer_addr() << " -- "
		      << *m << " -- " << m << " con "
  ...
  if (!m->get_priority())
    m->set_priority(hw_msgr->get_default_send_priority());

  m->get_header().src = hw_msgr->get_myname();
  ...
  if (hw_msgr->get_myaddr() == get_peer_addr()) { //loopback connection
    ldout(hw_msgr->cct, 20) << __func__ << " " << *m << " local" << dendl;
    std::lock_guard<std::mutex> l(write_lock);
    if (can_write != WriteStatus::CLOSED) {
      dispatch_queue->local_delivery(m, m->get_priority());
    } else {
      ldout(hw_msgr->cct, 10) << __func__ << " loopback connection closed."
  ...
  bool can_fast_prepare = hw_msgr->ms_can_fast_dispatch(m);
  if (can_fast_prepare)
  ...
  if (can_fast_prepare && (can_write == WriteStatus::NOWRITE || get_features() != f)) {
    ...
    bl.clear();
    m->get_payload().clear();
    ldout(hw_msgr->cct, 5) << __func__ << " clear encoded buffer previous "
                              << f << " != " << get_features() << dendl;
  }
  if (can_write == WriteStatus::CLOSED) {
    ldout(hw_msgr->cct, 10) << __func__ << " connection closed."
                               << " Drop message " << m << dendl;
    m->put();
  } else {
    m->trace.event("async enqueueing message");
    out_q[m->get_priority()].emplace_back(std::move(bl), m);
    ldout(hw_msgr->cct, 15) << __func__ << " inline write is denied, reschedule m=" << m << dendl;
...
void RDMAReliableConnected::requeue_sent()
{
  if (sent.empty())
    return;
  ...
  while (!sent.empty()) {
    Message* m = sent.back();
    sent.pop_back();
    ldout(hw_msgr->cct, 10) << __func__ << " " << *m << " for resend "
                               << " (" << m->get_seq() << ")" << dendl;
...
void RDMAReliableConnected::discard_requeued_up_to(uint64_t seq)
{
  ldout(hw_msgr->cct, 10) << __func__ << " " << seq << dendl;
  std::lock_guard<std::mutex> l(write_lock);
  ...
  while (!rq.empty()) {
    ...
    if (p.second->get_seq() == 0 || p.second->get_seq() > seq)
      break;
    ldout(hw_msgr->cct, 10) << __func__ << " " << *(p.second) << " for resend seq " << p.second->get_seq()
...
/*
 * Tears down the RDMAReliableConnected's message queues, and removes them from the DispatchQueue
 * Must hold write_lock prior to calling.
 */
void RDMAReliableConnected::discard_out_queue()
{
  ldout(hw_msgr->cct, 10) << __func__ << " started" << dendl;

  for (list<Message*>::iterator p = sent.begin(); p != sent.end(); ++p) {
    ldout(hw_msgr->cct, 20) << __func__ << " discard " << *p << dendl;
    (*p)->put();
  }
  sent.clear();
  for (map<int, list<pair<bufferlist, Message*> > >::iterator p = out_q.begin(); p != out_q.end(); ++p)
    for (list<pair<bufferlist, Message*> >::iterator r = p->second.begin(); r != p->second.end(); ++r) {
      ldout(hw_msgr->cct, 20) << __func__ << " discard " << r->second << dendl;
      r->second->put();
    }
  out_q.clear();
}

int RDMAReliableConnected::randomize_out_seq()
{
  if (get_features() & CEPH_FEATURE_MSG_AUTH) {
    ...
    rand_seq &= SEQ_MASK;
    lsubdout(hw_msgr->cct, ms, 10) << __func__ << " randomize_out_seq " << rand_seq << dendl;
    out_seq = rand_seq;
    return seq_error;
  } else {
...
void RDMAReliableConnected::fault()
{
  if (state == STATE_CLOSED || state == STATE_NONE) {
    ldout(hw_msgr->cct, 10) << __func__ << " connection is already closed" << dendl;
    return ;
  }

  if (policy.lossy && !(state >= STATE_CONNECTING && state < STATE_CONNECTING_READY)) {
    ldout(hw_msgr->cct, 1) << __func__ << " on lossy channel, failing" << dendl;
    _stop();
    dispatch_queue->queue_reset(this);
    return ;
  }
  ...
  if (!once_ready && !is_queued() &&
      ...
      !replacing) {
    ldout(hw_msgr->cct, 10) << __func__ << " with nothing to send and in the half "
  ...
  if (policy.standby && !is_queued() && state != STATE_WAIT) {
    ldout(hw_msgr->cct, 10) << __func__ << " with nothing to send, going to standby" << dendl;
    state = STATE_STANDBY;
    write_lock.unlock();
    return;
  }
  ...
  if (!(state >= STATE_CONNECTING && state < STATE_CONNECTING_READY) &&
    ...
    if (policy.server) {
      ldout(hw_msgr->cct, 0) << __func__ << " server, going to standby" << dendl;
      state = STATE_STANDBY;
    } else {
      ldout(hw_msgr->cct, 0) << __func__ << " initiating reconnect" << dendl;
      connect_seq++;
      state = STATE_CONNECTING;
    }
    backoff = utime_t();
    ...
    if (state == STATE_WAIT) {
      backoff.set_from_double(hw_msgr->cct->_conf->ms_max_backoff);
    } else if (backoff == utime_t()) {
      backoff.set_from_double(hw_msgr->cct->_conf->ms_initial_backoff);
    } else {
      backoff += backoff;
      if (backoff > hw_msgr->cct->_conf->ms_max_backoff)
        backoff.set_from_double(hw_msgr->cct->_conf->ms_max_backoff);
    }

    state = STATE_CONNECTING;
    ldout(hw_msgr->cct, 10) << __func__ << " waiting " << backoff << dendl;
    // woke up again;
...
void RDMAReliableConnected::was_session_reset()
{
  ldout(hw_msgr->cct,10) << __func__ << " started" << dendl;
  std::lock_guard<std::mutex> l(write_lock);
  if (delay_state)
  ...
  if (randomize_out_seq()) {
    ldout(hw_msgr->cct, 15) << __func__ << " could not get random bytes to set seq number for session reset; set seq number to " << out_seq << dendl;
  }
...
void RDMAReliableConnected::_stop()
{
  if (state == RC_State::STATE_CLOSED)
    return ;

  if (delay_state)
    delay_state->flush();

  ldout(hw_msgr->cct, 2) << __func__ << dendl;
  std::lock_guard<std::mutex> l(write_lock);

  reset_recv_state();
  dispatch_queue->discard_queue(conn_id);
  discard_out_queue();
  hw_msgr->unregister_conn(this);
  worker->release_worker();

  state = RC_State::STATE_CLOSED;
  open_write = false;
...
void RDMAReliableConnected::prepare_send_message(uint64_t features, Message *m, bufferlist &bl)
{
  ldout(hw_msgr->cct, 20) << __func__ << " m" << " " << *m << dendl;

  // associate message with Connection (for benefit of encode_payload)
  if (m->empty_payload())
    ldout(hw_msgr->cct, 20) << __func__ << " encoding features "
                               << features << " " << m << " " << *m << dendl;
  else
    ldout(hw_msgr->cct, 20) << __func__ << " half-reencoding features "
                               << features << " " << m << " " << *m << dendl;
...
ssize_t RDMAReliableConnected::write_message(Message *m, bufferlist& bl, bool more)
{
  FUNCTRACE();
  assert(center->in_thread());
  m->set_seq(++out_seq);
  ...
  // digital signature for the message, if the RDMAReliableConnected has session
  // security set up.  Some session security options do not
  // actually calculate and check the signature, but they should
  // handle the calls to sign_message and check_signature.  PLR
  if (session_security.get() == NULL) {
    ldout(hw_msgr->cct, 20) << __func__ << " no session security" << dendl;
  } else {
    if (session_security->sign_message(m)) {
      ldout(hw_msgr->cct, 20) << __func__ << " failed to sign m="
                                 << m << "): sig = " << footer.sig << dendl;
    } else {
      ldout(hw_msgr->cct, 20) << __func__ << " signed m=" << m
  ...
  ldout(hw_msgr->cct, 20) << __func__ << " sending message type=" << header.type
                             << " src " << entity_name_t(header.src)
                             << " front=" << header.front_len
                             << " data=" << header.data_len
                             << " off " << header.data_off << dendl;
  ...
  ldout(hw_msgr->cct, 20) << __func__ << " sending " << m->get_seq()
                             << " " << m << dendl;
  ssize_t total_send_size = outcoming_bl.length();
  ssize_t rc = _try_send(more);
  if (rc < 0) {
    ldout(hw_msgr->cct, 1) << __func__ << " error sending " << m << ", "
                              << cpp_strerror(rc) << dendl;
  } else if (rc == 0) {
    logger->inc(l_msgr_send_bytes, total_send_size - original_bl_len);
    ldout(hw_msgr->cct, 10) << __func__ << " sending " << m << " done." << dendl;
  } else {
    logger->inc(l_msgr_send_bytes, total_send_size - outcoming_bl.length());
    ldout(hw_msgr->cct, 10) << __func__ << " sending " << m << " continuely." << dendl;
  }
  if (m->get_type() == CEPH_MSG_OSD_OP)
...
void RDMAReliableConnected::reset_recv_state()
{
  ...
  if (state > STATE_OPEN_MESSAGE_THROTTLE_MESSAGE &&
    ...
    ldout(hw_msgr->cct, 10) << __func__ << " releasing " << 1
  ...
  if (state > STATE_OPEN_MESSAGE_THROTTLE_BYTES &&
      state <= STATE_OPEN_MESSAGE_READ_FOOTER_AND_DISPATCH) {
    if (policy.throttler_bytes) {
      ldout(hw_msgr->cct, 10) << __func__ << " releasing " << cur_msg_size
...
void RDMAReliableConnected::handle_ack(uint64_t seq)
{
  ldout(hw_msgr->cct, 15) << __func__ << " got ack seq " << seq << dendl;
  // trim sent list
  std::lock_guard<std::mutex> l(write_lock);
  while (!sent.empty() && sent.front()->get_seq() <= seq) {
    Message* m = sent.front();
    sent.pop_front();
    ldout(hw_msgr->cct, 10) << __func__ << " got ack seq "
                               << seq << " >= " << m->get_seq() << " on "
                               << m << " " << *m << dendl;
    m->put();
  }
...
void RDMAReliableConnected::DelayedDelivery::do_request(int id)
{
  Message *m = nullptr;
  {
...
void RDMAReliableConnected::DelayedDelivery::flush() {
  stop_dispatch = true;
  center->submit_to(
...
void RDMAReliableConnected::send_keepalive()
{
  ldout(hw_msgr->cct, 10) << __func__ << dendl;
  std::lock_guard<std::mutex> l(write_lock);
  if (can_write != WriteStatus::CLOSED) {
    keepalive = true;
    center->dispatch_event_external(write_handler);
...
void RDMAReliableConnected::mark_down()
{
  ldout(hw_msgr->cct, 1) << __func__ << dendl;
  std::lock_guard<std::mutex> l(lock);
  _stop();
}

void RDMAReliableConnected::_append_keepalive_or_ack(bool ack, utime_t *tp)
{
  ldout(hw_msgr->cct, 10) << __func__ << dendl;
  if (ack) {
    assert(tp);
...
void RDMAReliableConnected::handle_write()
{
  ldout(hw_msgr->cct, 10) << __func__ << dendl;
  ssize_t r = 0;
  ...
  if (can_write == WriteStatus::CANWRITE) {
    ...
    do {
      ...
      if (r < 0) {
        ldout(hw_msgr->cct, 1) << __func__ << " send msg failed" << dendl;
        goto fail;
      }
      write_lock.lock();
      if (r > 0)
    ...
    if (left) {
      ...
      ldout(hw_msgr->cct, 10) << __func__ << " try send msg ack, acked " << left << " messages" << dendl;
      ack_left -= left;
      left = ack_left;
      r = _try_send(left);
    } else if (is_queued()) {
    ...
    if (state == STATE_STANDBY && !policy.server && is_queued()) {
      ldout(hw_msgr->cct, 10) << __func__ << " policy.server is false" << dendl;
      _connect();
    } else if (cs && state != STATE_NONE && state != STATE_CONNECTING && state != STATE_CONNECTING_RE && state != STATE_CLOSED) {
      r = _try_send();
      if (r < 0) {
        ldout(hw_msgr->cct, 1) << __func__ << " send outcoming bl failed" << dendl;
        write_lock.unlock();
        fault();
        lock.unlock();
        return ;
...
void RDMAReliableConnected::wakeup_from(uint64_t id)
{
  lock.lock();
  register_time_events.erase(id);
  lock.unlock();
...
void RDMAReliableConnected::tick(uint64_t id)
{
  auto now = ceph::coarse_mono_clock::now();
  ldout(hw_msgr->cct, 20) << __func__ << " last_id=" << last_tick_id
  ...
  if (inactive_timeout_us < (uint64_t)idle_period) {
    ldout(hw_msgr->cct, 1) << __func__ << " idle(" << idle_period << ") more than "
                              << inactive_timeout_us
