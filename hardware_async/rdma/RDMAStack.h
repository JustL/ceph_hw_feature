...
class RDMADispatcher {
   ...
   */
  ceph::unordered_map<uint32_t, std::pair<QueuePair*, RDMASocket*> > qp_conns;
 ...
 public:
  ...
  void polling_stop();
  void polling();
  int register_qp(QueuePair *qp, RDMASocket* csi);
  ...
  RDMASocket* get_conn_lockless(uint32_t qp);
...
class RDMAWorker : public Worker {
  ...
  std::list<RDMASocket*> pending_sent_conns;
  ...
  // for creating different RDMA sockets (reliable connected(RC),
  //                                      unreliable_connected(UC),
  //                                      userdatagram(UD))
  class RDMASocketFactory
  {
    public:
      RDMASocketFactory();
      ~RDMASocketFactory();
      RDMASocket::RDMASocketType get_sock_type() const;
      void set_sock_type(const RDMASocket::RDMASocketType sock_type);
      RDMASocket* create_RDMA_socket(CephContext *cct, Infiniband *ib,
                                     RDMADispatcher *dp, RDMAWorker *wr);

    private:
      RDMASocket::RDMASocketType _sock_type;

  };

  RDMASocketFactory sock_factory;
 ...
 public:
  ...
  RDMAStack *get_stack() { return stack; }
  int get_reged_mem(RDMASocket *o, std::vector<Chunk*> &c, size_t bytes);
  void remove_pending_conn(RDMASocket *o) {
