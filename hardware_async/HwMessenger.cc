...
#include "rdma/RDMAConnFactory.h"
...
static ostream& _prefix(std::ostream *_dout, HwMessenger *m) {
...
Processor::Processor(HwMessenger *r, Worker *w, CephContext *c)
...
class C_handle_reap : public EventCallback {
  ...
  public:
  explicit C_handle_reap(HwMessenger *m): msgr(m) {}
  void do_request(int id) override {
...
HwConnectionFactory::HwConnectionFactory(const std::string &type)
: hw_type(type) 
{}

HwConnectionFactory::~HwConnectionFactory()
{}


HwConnectionFacotry* HwConnectionFactory::create_factory(const std::string &type)

{
  // Currently supports only RDMA connections. Other types can be 
  // added later


  if(type.find("rdma") != std::string::npos)
  {
    // RDMA supports three types:
    // 1) reliable connected; 
    // 2) unreliable connected;
    // 3) user datagram;
    return new RDMAConnFactory(type);
...
HwMessenger::HwMessenger(CephContext *cct, entity_name_t name,
                               const std::string &type, string mname, uint64_t _nonce)
  : SimplePolicyMessenger(cct, name,mname, _nonce),
    dispatch_queue(cct, this, mname),
    lock("HwMessenger::lock"),
    nonce(_nonce), need_addr(true), did_bind(false),
    global_seq(0), deleted_lock("HwMessenger::deleted_lock"),
    cluster_protocol(0), stopped(true), 
    conn_Factory(nullptr)
{
  ...
  cct->lookup_or_create_singleton_object<StackSingleton>(single, "HwMessenger::NetworkStack::"+transport_type);
...
int HwMessenger::bind(const entity_addr_t &bind_addr)
{
  lock.Lock();
...
int HwMessenger::rebind(const set<int>& avoid_ports)
{
...
int HwMessenger::client_bind(const entity_addr_t &bind_addr)
{
...
void HwMessenger::_finish_bind(const entity_addr_t& bind_addr,
...
HwConnectionRef HwMessenger::create_connect(const entity_addr_t& addr, int type)
{
...
ConnectionRef HwMessenger::get_connection(const entity_inst_t& dest)
{
  Mutex::Locker l(lock);
  ...
  HwConnectionRef conn = _lookup_conn(dest.addr);
  if (conn) {
...
ConnectionRef HwMessenger::get_loopback_connection()
{
  return local_connection;
}

int HwMessenger::_send_message(Message *m, const entity_inst_t& dest)
{
  FUNCTRACE();
  assert(m);
...
void HwMessenger::submit_message(Message *m, HwConnectionRef con,
 ...
 */
void HwMessenger::set_addr_unknowns(const entity_addr_t &addr)
{
  Mutex::Locker l(lock);
...
void HwMessenger::set_addr(const entity_addr_t &addr)
{
  Mutex::Locker l(lock);
...
void HwMessenger::shutdown_connections(bool queue_reset)
{
  ldout(cct,1) << __func__ << " " << dendl;
  lock.Lock();
  for (set<HwConnectionRef>::iterator q = accepting_conns.begin();
  ...
  while (!conns.empty()) {
    ceph::unordered_map<entity_addr_t, HwConnectionRef>::iterator it = conns.begin();
    HwConnectionRef p = it->second;
...

  {
    ...
    while (!deleted_conns.empty()) {
      set<HwConnectionRef>::iterator it = deleted_conns.begin();
      HwConnectionRef p = *it;
...
void HwMessenger::mark_down(const entity_addr_t& addr)
{
  lock.Lock();
  HwConnectionRef p = _lookup_conn(addr);
  if (p) {
...
int HwMessenger::get_proto_version(int peer_type, bool connect) const
{
...
void HwMessenger::learned_addr(const entity_addr_t &peer_addr_for_me)
{
