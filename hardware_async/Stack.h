...
class ConnectedSocketImpl {
 public:
  ...
  virtual bool can_read_memory const
  { return false; }
...
class ConnectedSocket {
  std::shared_ptr<ConnectedSocketImpl> _csi;
 ...
 public:
  ...
  // Add this in order to get access to the implementation of
  // the socket. We need this to use RDMA more efficiently.
  bool can_directly_read() const
  {
    return _csi->can_read_memory();
  }

  bool can_remotely_write() const
  {
    return _csi->can_remotely_write();
  }
